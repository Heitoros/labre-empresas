...
function parseTipoFonte(value: string): TipoFonte {
  const v = value.trim().toUpperCase();
  if (v === "PAV" || v === "NAO_PAV") return v;
  throw new Error(`--tipo invalido: "${value}". Use PAV ou NAO_PAV.`);
}

function detectarAnoPorArquivo(arquivo: string): number | undefined {
  const matches = arquivo.match(/(20\d{2})/g);
  if (!matches || matches.length === 0) return undefined;
  return Number(matches[matches.length - 1]);
}

function detectarMesPorArquivo(arquivo: string): number | undefined {
...
function lerLinhasTrechos(sheet: XLSX.WorkSheet): Record<string, unknown>[] {
  ...
  if (headerIdx === -1) {
    throw new Error("Nao foi possivel localizar o cabecalho da aba Trechos.");
  }

  const header = matriz[headerIdx].map((c) => String(c ?? "").trim());
  const rows = matriz.slice(headerIdx + 1);
...
async function main() {
  ...
  const tipoFonte = parseTipoFonte(requiredArg("--tipo"));

  const anoArg = getArg("--ano");
  const mesArg = getArg("--mes");
  const regiaoArg = getArg("--regiao");

  const ano = anoArg ? Number(anoArg) : detectarAnoPorArquivo(arquivo);
  const mes = mesArg ? Number(mesArg) : detectarMesPorArquivo(arquivo);
  const regiao = regiaoArg ? Number(regiaoArg) : detectarRegiaoPorArquivo(arquivo);

  const sheetNameArg = getArg("--sheet") ?? "Trechos";
  const limparAntes = getArg("--limparAntes") === "true";
  ...
  if (!sheet) {
    throw new Error(`A aba "${sheetNameArg}" nao existe. Abas: ${workbook.SheetNames.join(", ")}`);
  }

  const linhas = lerLinhasTrechos(sheet);
  if (!linhas.length) throw new Error("Nenhuma linha valida encontrada na aba Trechos.");

  const client = new ConvexHttpClient(convexUrl);
  const resultado = await client.mutation(api.trechos.importarTrechos, {
    tipoFonte,
    regiao,
    ano,
    mes,
